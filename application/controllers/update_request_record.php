...
function Update(){
  ...
  $data['assigned']=
  $this->db->select('assignment.analyst_name,assignment.sample_issued')->get_where('assignment', array('test_request_id' => $id))->result_array();
...
function reset(){
  $tr_id=$this->uri->segment(3);
    ...
    if($this->input->post('submit')){
        $this->update_requestmodel->reset($tr_id);                
    }
