<?php
class Oxidisable extends CI_Controller {

    function __construct() {
        parent::__construct();
    }
     function Oxidisable_specifications() {
    ...
    $data['oxidisable']=
    $this->db->select('*')->get_where('oxidisable', array('test_request_id' => $test_request_id))->result_array();
    ...
    $this->load->view('general_tests/oxidisable_specifications',$data);
    ...
    $this->load->view('general_tests/oxidisable_specifications_view',$data);
    $this->load->helper(array('form'));
    }
    ...
    $this->load->view('general_tests/oxidisable_view',$data);
    $this->load->helper(array('form'));
    }
    function oxidisable_complete_worksheet() {
    ...
    $data['oxidisable']=$this->db->select('*')->get_where('oxidisable', array('test_request_id' => $test_request_id))->result_array();
    
    $query=$this->db->select('test_request.id AS tr,test_request.client_id,test_request.active_ingredients,test_request.brand_name,test_request.applicant_address,test_request.date_time,test_request.manufacturer_name,test_request.manufacturer_address,test_request.batch_lot_number,
    test_request.sample_source,test_request.expiry_date,test_request.date_manufactured,test_request.quantity_type,test_request.sample_source,test_request.laboratory_number,test_request.applicant_name,
    test_request.quantity_remaining,test_request.quantity_submitted,test_request.reference_number,test_request.applicant_ref_number,test_request.identification,test_request.friability,test_request.acidity_alkalinity,test_request.dissolution,test_request.assay,test_request.test_specification,
    test_request.disintegration,test_request.oxidisable,test_request.ph_alkalinity,test_request.full_monograph,test_request.content_uniformity,test_request.microbiology,test_request.request_status')->get_where('test_request', array('id' => $test_request_id));
     
    $results=$query->result_array();
    $data['query']=$results[0];
       
    $this->load->view('general_tests/oxidisable_complete_worksheet_view',$data);
    $this->load->helper(array('form'));
    }
    function save(){
        $this->load->model('oxidisable_model');

        if ($this->input->post('submit')) {
            $this->oxidisable_model->process();
        }
	}
     function save_oxidisable_specifications(){
       $this->load->model('oxidisable_model');

        if ($this->input->post('submit')) {
            $this->oxidisable_model->process_specifications();
        }                
    }
